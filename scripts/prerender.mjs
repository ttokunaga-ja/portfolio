...
function shellPathFor(routePath) {
  const section = routePath.split("/").filter(Boolean)[0] ?? "";
...
const routePaths = getStaticPathsForPrerender();
const templates = new Map();

for (const routePath of routePaths) {
  const shellPath = shellPathFor(routePath);
...
for (const routePath of routePaths) {
  const shell = templates.get(shellPathFor(routePath));
  const rendered = render(routePath, "ja");
  rendered.routePath = routePath;
